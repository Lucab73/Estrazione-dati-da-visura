...
def estrai_dati(filepath):
    ...
    # --- 1. Ricerca della "Forma giuridica" (CON CONTROLLO SAFE) ---
    ...
    for i, riga in enumerate(righe):
        if "Forma giuridica" in riga:
            ...
            # (parti[2:] assume che "Forma giuridica" siano le prime 2 parole, 
            # se la riga inizia con altro potrebbe servire logica più complessa, ma ok per ora)
            if len(parti) > 2:
                for parola in parti[2:]:
                    if parola and parola[0].isupper():  # Se trovi una parola che inizia con maiuscola, interrompi
                        trovato_forma = True
                        break
                    forma_giuridica_parole.append(parola)

            # Se non è completa, continua con la riga successiva
            # *** MODIFICA: Controllo rigoroso se esiste riga successiva ***
            ...
            # Se dopo tutto questo è vuota, mettiamo un placeholder per evitare errori visivi
            if not forma_giuridica:
                forma_giuridica = "NON IDENTIFICATA"
            break

    # --- 2. Estrarre il numero degli addetti ---
    ...
    # --- 3. Estrarre la ragione sociale (CON CONTROLLO SAFE) ---
    ragione_sociale = ""
    for i, riga in enumerate(righe):
        if "VISURA" in riga or "FASCICOLO" in riga:
            ...
            # *** MODIFICA: Controllo se l'indice 'inizio' esiste ***
            if inizio < len(righe):
                # Verifica se la riga iniziale è vuota
                if righe[inizio].strip() == "":
                    inizio += 1
                
                # Ricontrolliamo l'indice dopo l'incremento
                if inizio < len(righe):
                ...
                # Se siamo alla fine del file, usiamo un fallback o lasciamo vuoto
                ragione_sociale = "NON TROVATA (Fine file)"
                break

    # --- 4. Estrarre l'indirizzo (Comune e Via) (CON CONTROLLO SAFE) ---
    comune = "NON TROVATO"
    via = "NON TROVATO"
    ...
    for i, riga in enumerate(righe):
        if "Indirizzo Sede" in riga:
            ...
            # Controllo lunghezza parti per sicurezza
            if len(parti) > 2:
                for parola in parti[2:]:  # Ignora "Indirizzo Sede"
                    if not trovato_comune:
                        ...
                        if parola and parola[0].isupper():
            ...
            # *** MODIFICA: Controllo esplicito riga successiva ***
            if i + 1 < len(righe):
                riga_successiva = righe[i + 1]
                parti_successiva = riga_successiva.split()
                for parola in parti_successiva:
    ...
    # --- Fine controlli modificati ---

    # Lista delle sezioni che determinano la fine della ricerca
    sezioni_fine = [
        "Trasferimenti d'azienda, fusioni, scissioni, subentri",
        "Trasferimenti d'azienda, subentri", # Corretto virgola mancante nel tuo codice originale
    ...
    if not occorrenze_sezioni:
        # Se non trova la seconda occorrenza, prova a non limitare o usa un warning
        # print("Non è stata trovata una seconda occorrenza di nessuna delle sezioni di fine.")
        # Non ritorniamo lista vuota, proviamo a processare tutto il testo se fallisce il taglio,
        # oppure impostiamo la fine alla fine del file.
        riga_fine = len(righe)
    else:
    ...
    def verifica_cognome(nome, codice_fiscale):
        ...
        """
        if len(codice_fiscale) < 6: return False
        ...
        if terza_parola and all(lettera in terza_parola for lettera in successive_3_lettere):
            return False
        ...
        if all(lettera in seconda_parola for lettera in successive_3_lettere) or \
                (terza_parola and all(lettera in terza_parola for lettera in quinto_sesto_carattere)) or \
    ...
    def elabora_sezione(testo_sezione, tipo_sezione):
        righe_sezione = testo_sezione.splitlines()

        for i, riga in enumerate(righe_sezione):
            ...
            if match_cf:
                ...
                for offset in offsets_da_provare:
                    index = i + offset
                    if 0 <= index < len(righe_sezione):
                        riga_corrente = rimuovi_numeri(righe_sezione[index].strip())
                        if not riga_corrente:
                            continue

                        # La prima parola deve iniziare con una maiuscola, altrimenti ignoriamo la riga
                        parti_riga = riga_corrente.split()
                        if not parti_riga or not parti_riga[0].isupper():
                            continue
